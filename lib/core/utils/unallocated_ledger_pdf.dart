...
class UnallocatedLedgerPdf {
  ...
  static const primaryColor = PdfColor.fromInt(0xFF00695C); // استخدمنا لوناً مختلفاً (Teal) لتمييز المحفظة
  ...
  static Future<Uint8List> generatePdf({
    ...
    pdf.addPage(
      pw.MultiPage(
          ...
          pw.Row(
            ...
            children:[
              ...
              pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children:[
                pw.Text('كشف حساب استثماري (محفظة)', style: pw.TextStyle(font: arabicBoldFont, fontSize: 14, color: accentColor, letterSpacing: 1)),
                pw.SizedBox(height: 4),
                pw.Container(
        ...
        build: (context) =>[
          buildSectionTitle('بيانات المستثمر (الفريق الثاني)', const pw.IconData(0xe7fd)),
          pw.Container(
            decoration: pw.BoxDecoration(border: pw.Border.all(color: borderColor), borderRadius: pw.BorderRadius.circular(6)),
            child: pw.Column(children:[
              buildGridRow(['اسم المستثمر', 'رقم الهاتف', 'الرقم الوطني', 'كود العميل'],[client.name, client.phone, client.nationalId ?? 'غير مدون', client.id.split('-').first.toUpperCase()]),
            ])
          ),

          buildSectionTitle('التفاصيل التعاقدية للمحفظة الاستثمارية', const pw.IconData(0xe873)),
          pw.Container(
            decoration: pw.BoxDecoration(border: pw.Border.all(color: borderColor), borderRadius: pw.BorderRadius.circular(6)),
            child: pw.Column(children:[
              buildGridRow(['تاريخ فتح المحفظة', 'طبيعة العقد', 'الدفعة المقدمة', 'اسم الكفيل'],[_formatDate(contract.contractDate), 'أسهم غير مخصصة (يتم التخصيص لاحقاً)', '${_formatWithCommas(contract.downPayment)} ل.س', contract.guarantorName]),
            ])
          ),
          ...
          pw.Container(
            ...
            child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceAround, children:[
              buildSummaryCol('إجمالي المبالغ المُستثمرة (المسددة)', '${_formatWithCommas(totalPaid)} ل.س', PdfColors.green400),
              pw.Container(width: 1, height: 30, color: PdfColors.grey500),
              buildSummaryCol('إجمالي الأمتار المكتسبة في المحفظة', '${totalMeters.toStringAsFixed(3)} م²', PdfColors.white),
            ]),
          ),
          ...
          pw.Table(
            ...
            children:[
              pw.TableRow(
                ...
                children:['رقم الإيصال', 'تاريخ الدفع', 'المبلغ (ل.س)', 'سعر المتر المعتمد', 'البونص %', 'الأمتار المكتسبة'].map((text) => pw.Padding(
