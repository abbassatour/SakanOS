...
class MaterialsTrendCubit extends Cubit<MaterialsTrendState> {
  MaterialsTrendCubit(this._erpRepository) : super(MaterialsTrendState(referenceDate: DateTime.now()));
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      emit(state.copyWith(status: MaterialsTrendStatus.failure, errorMessage: e.toString()));
    }
  }
  ...
  void _processAndEmitData() {
    ...
    void initBucket(String key) {
      tempIron[key] = []; tempCement[key] = []; tempBlock[key] =[];
      tempFormwork[key] = []; tempAgg[key] = []; tempWorker[key] =[];
    }

    if (state.timeFilter == TimeFilter.daily) {
      for (int i = 6; i >= 0; i--) { initBucket(DateFormat('MM-dd').format(refDate.subtract(Duration(days: i)))); }
    } else if (state.timeFilter == TimeFilter.weekly) {
      for (int i = 1; i <= 4; i++) { initBucket('الأسبوع $i'); }
    } else if (state.timeFilter == TimeFilter.monthly) {
      for (int i = 1; i <= 12; i++) { initBucket('${refDate.year}-${i.toString().padLeft(2, '0')}'); }
    } else if (state.timeFilter == TimeFilter.yearly) {
      for (int i = 4; i >= 0; i--) { initBucket('${refDate.year - i}'); }
    }
    ...
    for (var p in _cachedPrices) {
      ...
      } else if (state.timeFilter == TimeFilter.weekly && p.effectiveDate.year == refDate.year && p.effectiveDate.month == refDate.month) {
        int weekNum = ((p.effectiveDate.day - 1) / 7).floor() + 1;
        key = 'الأسبوع ${weekNum > 4 ? 4 : weekNum}';
      } else if (state.timeFilter == TimeFilter.monthly && p.effectiveDate.year == refDate.year) {
        key = '${p.effectiveDate.year}-${p.effectiveDate.month.toString().padLeft(2, '0')}';
      } else if (state.timeFilter == TimeFilter.yearly) {
    ...
    Map<String, double> processMap(Map<String, List<double>> tempMap) {
      ...
      tempMap.forEach((k, v) => finalMap[k] = v.isEmpty ? 0.0 : v.fold(0.0, (a, b) => a + b) / v.length);
      
      double lastKnown = 0.0;
      for (var val in finalMap.values) { if (val > 0) { lastKnown = val; break; } }
      for (var k in finalMap.keys) {
        if (finalMap[k] == 0.0) finalMap[k] = lastKnown;
        else lastKnown = finalMap[k]!;
      }
      return finalMap;
    }
