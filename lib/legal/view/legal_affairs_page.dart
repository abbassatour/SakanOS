...
class _LegalAffairsViewState extends State<LegalAffairsView> {
                          ...
                          Card(
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: ConstrainedBox(
                                ...
                                child: DataTable(
                                  columnSpacing: 22, horizontalMargin: 20,
                                  headingRowColor: WidgetStateProperty.all(Colors.brown.shade50),
                                  dataRowMinHeight: 60, dataRowMaxHeight: 80,
                                  columns: const[
                                    DataColumn(label: Text('نوع الإجراء', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                    DataColumn(label: Text('العميل / العقار', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                    DataColumn(label: Text('التاريخ الفعلي', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                    DataColumn(label: Text('المرفقات', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                    DataColumn(label: Text('بواسطة', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                    DataColumn(label: Text('إدارة', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown))),
                                  ],
                                  rows: filteredActions.asMap().entries.map((entry) {
                                    ...
                                    return DataRow(
                                      color: WidgetStateProperty.resolveWith<Color?>((Set<WidgetState> s) => entry.key.isEven ? Colors.grey.withOpacity(0.03) : null),
                                      cells:[
  ...
  Widget _buildSearchBar(int count) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, boxShadow:[BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 4, offset: const Offset(0, 2))]),
      child: Row(
        children:[
          Expanded(
            child: SizedBox(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true, fillColor: Colors.grey.shade50, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 0),
                  ...
                  suffixIcon: _searchQuery.isNotEmpty ? IconButton(icon: const Icon(Icons.clear, color: Colors.grey, size: 20), onPressed: () { setState(() => _searchQuery = ''); FocusScope.of(context).unfocus(); }) : null,
                ),
              ),
            ),
          ),
          ...
          Container(
            height: 48, alignment: Alignment.center, padding: const EdgeInsets.symmetric(horizontal: 16),
            ...
            child: Text('$count إجراء', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown.shade700, fontSize: 14)),
          )
        ],
      ),
    );
