            ...
            return TabBarView(
              children:[
                ...
                _buildList(
                  ...
                  getSubtitle: (item) => 'رقم الهاتف: ${item.phone} ${item.nationalId != null ? ' | الرقم الوطني: ${item.nationalId}' : ''}',
                  getDeletedBy: (item) => state.userNamesMap[item.userId] ?? 'مجهول',
                ...
                _buildList(
                  ...
                  canRestore: canRestore,
                  canHardDelete: canHardDelete,
                  getTitle: (item) => '${item.unitType == 'shop' ? 'محل' : 'شقة'} رقم: ${item.apartmentNumber}', 
                  getSubtitle: (item) {
                    final bldgs = state.referenceBuildings.where((b) => b.id == item.buildingId);
                    final bldgName = bldgs.isNotEmpty ? bldgs.first.name : 'محضر محذوف';
                ...
                _buildList(
                  ...
                  getSubtitle: (item) {
                    final clients = state.referenceClients.where((c) => c.id == item.clientId);
                    final clientName = clients.isNotEmpty ? clients.first.name : 'عميل محذوف';
                ...
                _buildList(
                  ...
                  canRestore: canRestore,
                  canHardDelete: canHardDelete,
                  getTitle: (item) => 'إيصال مبلغ: ${formatWithCommas(item.amountPaid)} ل.س', 
                  getSubtitle: (item) {
                    final contracts = state.referenceContracts.where((c) => c.id == item.contractId);
                    final contract = contracts.isNotEmpty ? contracts.first : null;
                    if (contract != null) {
                      final clients = state.referenceClients.where((c) => c.id == contract.clientId);
                      final clientName = clients.isNotEmpty ? clients.first.name : 'عميل محذوف';
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: BorderSide(color: Colors.red.shade100, width: 1.5), // إطار أحمر خفيف يدل على الحذف
          ),
          child: Padding(
                  ...
                  children:[
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children:[
                          Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, decoration: TextDecoration.lineThrough, color: Colors.black87)),
                    ...
                    Container(
                      ...
                      child: Text('⏳ باقي $daysLeft أيام', style: TextStyle(color: Colors.orange.shade800, fontWeight: FontWeight.bold, fontSize: 12)),
                    )
                  ],
                ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children:[
                          Row(
                            children:[
                              Icon(Icons.delete_sweep, size: 14, color: Colors.red.shade400),
                              const SizedBox(width: 4),
                              const Text('حُذف بواسطة: ', style: TextStyle(fontSize: 12, color: Colors.grey)),
                              Text(deletedBy, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.red.shade700)),
                            ],
                          ),
                          const SizedBox(height: 4),
                          Row(
                      ...
                      children:[
                        if (canRestore)
                          TextButton.icon(
                            ...
                            style: TextButton.styleFrom(foregroundColor: Colors.green.shade700, padding: const EdgeInsets.symmetric(horizontal: 12)),
                            onPressed: () {
                              onRestore(item);
                        ...
                        if (canHardDelete)
                          TextButton.icon(
                            ...
                            style: TextButton.styleFrom(foregroundColor: Colors.red.shade700, padding: const EdgeInsets.symmetric(horizontal: 12)),
                            onPressed: () {
