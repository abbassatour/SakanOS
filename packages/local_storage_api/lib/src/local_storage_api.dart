...
class LocalStorageApi {
  ...
  Future<String> addBuilding(BuildingsCompanion building) =>
      _db.insertBuilding(building);

  // ==========================================
  // 🚪 الشقق (Apartments)
  // ==========================================
  Future<List<Apartment>> getAllApartments() => _db.getAllActiveApartments();
  Future<List<Apartment>> getApartmentsByBuilding(String buildingId) =>
      _db.getApartmentsForBuilding(buildingId);
  Future<String> addApartment(ApartmentsCompanion apartment) =>
      _db.insertApartment(apartment);
  ...
  Future<void> deleteClient(String id, String userId) =>
      _db.softDeleteClient(id, userId);
  ...
  Future<List<PaymentsLedgerData>> getContractLedger(String contractId) =>
      _db.getLedgerForContract(contractId);
  Future<String> addLedgerEntry(PaymentsLedgerCompanion entry) =>
      _db.insertLedgerEntry(entry);
  Future<int> updateWhatsAppStatus(String entryId, String userId) =>
      _db.markWhatsAppAsSent(entryId, userId);
  Future<List<PaymentsLedgerData>> getAllPayments() =>
      _db.getAllActivePayments();

  // ==========================================
  // ⚙️ الإعدادات والأسعار
  // ==========================================
  Future<MaterialPricesHistoryData?> getLatestPrices() => _db.getLatestPrices();
  Future<String> savePrices(MaterialPricesHistoryCompanion prices) =>
      _db.insertMaterialPriceRecord(prices);
  Stream<MaterialPricesHistoryData?> watchLatestPrices() =>
      _db.watchLatestPrices();
  Future<List<MaterialPricesHistoryData>> getAllMaterialPricesHistory() =>
      _db.getAllMaterialPricesHistory();
  ...
  Future<List<InstallmentsScheduleData>> getContractSchedule(
    String contractId,
  ) => _db.getScheduleForContract(contractId);
  ...
  Future<void> restructureContractSchedule({
    ...
    required String userId,
  }) => _db.restructureContractSchedule(
  ...
  // ==========================================
  Future<void> syncClient(ClientsCompanion c) =>
      _db.into(_db.clients).insertOnConflictUpdate(c);
  Future<void> syncContract(ContractsCompanion c) =>
      _db.into(_db.contracts).insertOnConflictUpdate(c);
  Future<void> syncPrice(MaterialPricesHistoryCompanion c) =>
      _db.into(_db.materialPricesHistory).insertOnConflictUpdate(c);
  Future<void> syncSchedule(InstallmentsScheduleCompanion c) =>
      _db.into(_db.installmentsSchedule).insertOnConflictUpdate(c);
  Future<void> syncPayment(PaymentsLedgerCompanion c) =>
      _db.into(_db.paymentsLedger).insertOnConflictUpdate(c);

  Future<void> syncBuilding(BuildingsCompanion c) =>
      _db.into(_db.buildings).insertOnConflictUpdate(c);
  Future<void> syncApartment(ApartmentsCompanion c) =>
      _db.into(_db.apartments).insertOnConflictUpdate(c);

  // ==========================================
  // 🗑️ دوال سلة المحذوفات (العملاء)
  // ==========================================
  Future<List<Client>> getDeletedClients() => _db.getDeletedClients();
  Future<void> restoreClient(String id, String userId) =>
      _db.restoreSoftDeletedClient(id, userId);
  ...
  Future<int> softDeleteLedgerEntry(String id, String userId) =>
      _db.softDeleteLedgerEntry(id, userId);
  Future<List<PaymentsLedgerData>> getDeletedLedgerEntries() =>
      _db.getDeletedLedgerEntries();
  Future<int> restoreLedgerEntry(String id, String userId) =>
      _db.restoreLedgerEntry(id, userId);
  Future<int> forceHardDeleteLedgerEntry(String id) =>
      _db.forceHardDeleteLedgerEntry(id);
  Future<void> autoCleanOldDeletedLedgerEntries() =>
      _db.autoCleanOldDeletedLedgerEntries();
  ...
  Future<List<LegalAction>> getLegalActionsForContract(String contractId) =>
      _db.getLegalActionsForContract(contractId);

  Future<String> addLegalAction(LegalActionsCompanion action) =>
      _db.insertLegalAction(action);

  // 🌟 سطر التعديل
  Future<void> updateLegalAction(LegalActionsCompanion action) =>
      _db.updateLegalAction(action);

  Future<int> deleteLegalAction(String id, String userId) =>
      _db.softDeleteLegalAction(id, userId);

  // ==========================================
  // 🕒 دوال تتبع النشاطات (Activity Log)
  // ==========================================
  Future<List<PaymentsLedgerData>> getRecentPayments(int limit) =>
      _db.getRecentPayments(limit);
  Future<List<Contract>> getRecentContracts(int limit) =>
      _db.getRecentContracts(limit);
  Future<List<Client>> getRecentClients(int limit) =>
      _db.getRecentClients(limit);
  ...
  Future<String> addLegalActionAttachment(
  ...
  Future<List<LegalActionAttachment>> getAllLegalActionAttachments() =>
      _db.getAllLegalActionAttachments();

  // ==========================================
  // 💵 أسعار الدولار
  // ==========================================
  Future<DollarPricesHistoryData?> getLatestDollarPrice() =>
      _db.getLatestDollarPrice();
  Future<String> saveDollarPrice(DollarPricesHistoryCompanion prices) =>
      _db.insertDollarPriceRecord(prices);
  Stream<DollarPricesHistoryData?> watchLatestDollarPrice() =>
      _db.watchLatestDollarPrice();
  Future<List<DollarPricesHistoryData>> getAllDollarPricesHistory() =>
      _db.getAllDollarPricesHistory();

  // ولا تنسَ إضافتها في قسم Rights / Sync Upserts في نفس الملف:
  Future<void> syncDollarPrice(DollarPricesHistoryCompanion c) =>
      _db.syncDollarPrice(c);
